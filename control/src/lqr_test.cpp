...
public:
    MobileRobotController() : Node("mobile_robot_controller")
    {
        ...
        desired_state_ = {0.0, 0.0, 0}; // x, y, and theta
        dt_ = 0.03;
        tolerance = 0.03;
...
private:
    ...
    void controlLoopCallback()
    {
        ...
        { // if robot has not reached goal
            ...
            pubVel(u.v, u.w);
            RCLCPP_INFO(rclcpp::get_logger("LQR"), "Desired State: (%f, %f, %f)", desired_state_.x, desired_state_.y, desired_state_.theta);
            RCLCPP_INFO(rclcpp::get_logger("LQR"), "Actual Error: (%f, %f, %f)", actual_state_.x, actual_state_.y, actual_state_.theta);
            // RCLCPP_INFO(rclcpp::get_logger("LQR"), "State Error: (%f, %f, %f)", state_error_.x, state_error_.y, state_error_.theta);
    ...
    void localPoseCallback(const nav_msgs::msg::Path::SharedPtr msg)
    {
        try
        {
            if (end_controller == false)
            {
                tf2::Quaternion quat(msg->poses[0].pose.orientation.x, msg->poses[0].pose.orientation.y, msg->poses[0].pose.orientation.z, msg->poses[0].pose.orientation.w);
                tf2::Matrix3x3 mat(quat);
                double roll, pitch, yaw;
                mat.getRPY(roll, pitch, yaw);
                desired_state_ = State(msg->poses[0].pose.position.x, msg->poses[0].pose.position.y, yaw);
                std::cout << desired_state_.x;
                RCLCPP_INFO(rclcpp::get_logger("LQR"), "Desired State: (%f, %f, %f)", desired_state_.x, desired_state_.y, desired_state_.theta);
            }
            else
            {
